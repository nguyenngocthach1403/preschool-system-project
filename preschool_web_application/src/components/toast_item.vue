<template> 
   <div :id="typeToast" class="toast-item container w-[300px] h-[80px] bg-white rounded-md" :class="{'leave-active': isCloseActive}">
        <div class="w-full h-full content-center px-[10px]">
            <!--Ttitle Toast-->
            <div class="title-toast text-[15px]">{{ toastItemData.title }}</div>
            <div class="title-toast text-[13px] text-gray-500">{{ toastItemData.content }}</div>

        </div>
        <button class="absolute w-[15px] right-1 top-1 hover:bg-gray-200" @click="closeToast"><img :src="close_icon" alt="" class="w-full"></button>
        <div id="time-line" class="h-2 bg-black absolute bottom-0 left-0" :style="{ width: + widthOfTimeLine + 'px'}"></div>
    </div>
</template>
...
<script setup>  
    ...
    function closeToast() {
        //close toast
            ...
            setTimeout(() => {
                emits('closeToast', drops.toastItemData.id)
            }, 200)
    }
