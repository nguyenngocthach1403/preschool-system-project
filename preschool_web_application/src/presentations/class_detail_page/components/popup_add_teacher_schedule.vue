<template>
  <PopupLayout
    :title="'Thêm giáo viên quản lý hoạt động'"
    @close="$emit('close')"
    class="absolute top-0 left-0"
  >
    <template #content>
      <div class="w-[800px] px-10 text-[17px]">
        <div
          class="w-full py-3 bg-white drop-shadow-xl rounded-md text-start px-5"
        >
          <span class="text-[19px] font-bold">{{
            getDayOfDate(new Date(props.scheduleData.date).getDay())
          }}</span>
          <p>
            Ngày:
            <span class="font-bold">
              {{
                ddmmyyyyDateString(
                  new Date(props.scheduleData.date).toLocaleDateString()
                )
              }}
            </span>
          </p>
          <p>
            Thời gian:
            <span class="font-bold">
              {{ props.scheduleData.timeTable.start_time }} -
              {{ props.scheduleData.timeTable.end_time }}
            </span>
          </p>
        </div>
        <div class="flex gap-2 flex-wrap">
          <div
            v-for="act in props.scheduleData.timeTable.activities"
            :key="act"
            class="w-fit py-3 bg-white drop-shadow-xl rounded-md text-start px-5 my-1"
          >
            {{ act.activity_name }}
          </div>
        </div>
...
async function updateTimeTable() {
  try {
    if (!props.scheduleData.timeTable.id) {
    ...
    const response = await ScheduleService.updateSchedule(
      props.scheduleData.timeTable.id,
      dataToUpDate
    );
