<template>
  <div class="bg-white ml-4 rounded-3xl text-center">
    ...
    >
      <div id="head">Chỉnh sửa SlideShow</div>
      <div id="site" class="flex px-[30px] py-[30px]">
        <div class="w-full my-3">
          <label>
            ...
            >
              <img :src="imageUpload" class="h-full w-full" />
              <input
                type="file"
...
async function getSlide() {
  const slideId = router.currentRoute.value.params.id;
  const response = await SlideshowService.getSlideById(slideId);
  ...
  const slide = response.data.data[0];

  if (slide) {
    imageUpload.value = slide.url;
...
async function updateSlide() {
  ...
  const response = await SlideshowService.updateSlide(id, formData);
