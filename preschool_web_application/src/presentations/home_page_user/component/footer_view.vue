<template>
  <footer class="bg-gray-800 text-white py-8 px-4 flex flex-col justify-center">
    <div class="flex flex-row justify-between mt-8">
      <div class="flex flex-col items-start ml-20">
        <h3 class="text-lg font-bold">Giới thiệu</h3>
        <router-link :to="{ name: 'Home' }" class="text-base hover:underline"
          >Trang chủ</router-link
        >
        <router-link
          :to="{ name: 'IntroduceView' }"
          class="text-base mt-1 hover:underline"
          >Giới thiệu</router-link
        >
        <router-link
          :to="{ name: 'RegistrationFormView' }"
          class="text-base mt-1 hover:underline"
          >Tuyển sinh</router-link
        >
        <router-link
          :to="{ name: 'NewsView' }"
          class="text-base mt-1 hover:underline"
          >Tin tức</router-link
        >
      </div>

      <div class="flex flex-col items-start">
        <h3 class="text-lg font-bold">Liên hệ</h3>
        <p class="text-base">
          Địa chỉ: 65 Huỳnh Thúc Kháng, Phường Bến Nghé, Quận 1, TP. Hồ Chí Minh
        </p>
        <a
          href="mailto:info@kindycity.edu.vn"
          class="text-base mt-1 hover:underline"
          >Email: toan@caothang.edu.vn</a
        >
        <a href="tel:0901853353" class="text-base mt-1 hover:underline"
          >Hotline: 0901-853-353</a
        >
      </div>

      <div class="flex flex-col items-start mr-20">
        <h3 class="text-lg font-bold">Liên kết</h3>
        <a href="#" class="text-base mt-1 hover:underline">Trang web</a>
        <a href="#" class="text-base mt-1 hover:underline">Facebook</a>
        <a href="#" class="text-base mt-1 hover:underline">Youtube</a>
        <a href="#" class="text-base mt-1 hover:underline">Instagram</a>
      </div>
    </div>
  </footer>
</template>

<script setup></script>

<style scoped></style>
