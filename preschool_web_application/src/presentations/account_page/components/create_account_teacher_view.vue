  ...
  <Layout :title="'Tạo tài khoản'">
    <template #content>
      <form class="w-full h-full px-[40px] text-start">
        ...
        <div id="Email" v-if="!$props.teacherId" class="w-full">
          <label for="">
...
onMounted(() => {
  if (drops.teacherId) {
    console.log(drops.teacherId);
    phoneInput.value = drops.teacherId.phone;
    emailInput.value = drops.teacherId.email;
    role.value = 3;
  }
});
...
async function createAccount() {
  ...
  if (drops.teacherId) {
    accountToCreate.role = 3;
    accountToCreate.teacherId = drops.teacherId.id;
  } else {
